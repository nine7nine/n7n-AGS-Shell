...
const Taskbar = () => Widget.Box({
    ...
    binds: [['children', Hyprland, 'clients', c => c.map(client => {
        for (const appName of options.desktop.dock.pinned_apps.value) {
            if (client.class.toLowerCase().includes(appName.toLowerCase()) || client.title && client.title.toLowerCase().includes(appName.toLowerCase()))
                return null;
        }
        ...
        .map(({ app, term = true }) => AppButton({
            ...
            on_primary_click: () => {
                for (const client of Hyprland.clients) {
                    if ((client.class.toLowerCase().includes(term) || client.title && client.title.toLowerCase().includes(term)) || client.title && client.title.toLowerCase().includes(term))
                        return focus(client);
                }
            ...
            connections: [[Hyprland, button => {
                const running = Hyprland.clients
                    .filter(client => (client.class.toLowerCase().includes(term) || client.title && client.title.toLowerCase().includes(term)));
